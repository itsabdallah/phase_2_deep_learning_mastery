...
for name, opt_fn in optimizers.items():
    set_seed(42)  # fairness across optimizers
    results[name] = run_experiment(name, opt_fn, lr=1e-3, epochs=10)
...
for name, (train_losses, _, _) in results.items():
    ax.plot(train_losses, label=name)
...
fig.tight_layout()
save_path = os.path.join(REPORT_DIR, "optimizer_training_loss.png")
